...
PHP_INI_CONF=${PHP_INI_CONF:-disable}
PHP_CLI=${PHP_CLI:-disable}
PHP_CLI_COMMAND=${PHP_CLI_COMMAND:-"php -a"}
OPCACHE=${OPCACHE:-disable}
EVENT=${EVENT:-disable}
REDIS=${REDIS:-enable}
MONGODB=${MONGODB:-disable}
MEMCACHE=${MEMCACHE:-enable}
SWOOLE=${SWOOLE:-disable}
...
PHP_SESSION_DIR=${PHP_SESSION_DIR:-"/data/wwwroot/session"}
...
if [[ $PHP_INI_CONF =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	...
	[ -f ${PHP_SESSION_DIR} ] && { mkdir -p ${PHP_SESSION_DIR} && chown -R www:www ${PHP_SESSION_DIR}; }
	sed -i "s@^;session.save_path = .*@session.save_path = \"${PHP_SESSION_DIR}\"@" ${PHP_INI_DIR}/php.ini
fi

if [[ "${OPCACHE}" =~ [eE][nN][aA][bB][lL][eE] ]]; then
	cat > ${PHP_INI_DIR}/conf.d/10-opcache.ini <<-EOF
		[opcache]
...
if [[ "${EVENT}" =~ [eE][nN][aA][bB][lL][eE] ]]; then
	cat > ${PHP_INI_DIR}/conf.d/10-event.ini <<-EOF
		[event]
		extension=event.so
	EOF
fi
...
if [[ "$MEMCACHE" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	#echo 'extension=memcache.so' > ${PHP_INI_DIR}/conf.d/10-memcache.ini
	cat > ${PHP_INI_DIR}/conf.d/10-memcached.ini <<-EOF
		[memcached]
...
if [[ "$MONGODB" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	cat > ${PHP_INI_DIR}/conf.d/10-mongodb.ini <<-EOF
		[mongodb]
...
if [[ "$REDIS" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	cat > ${PHP_INI_DIR}/conf.d/10-redis.ini <<-EOF
		[redis]
		extension=redis.so
	EOF
fi
...
if [[ "${SWOOLE}" =~ ^[eE][nN][aA][bB][lL][eE]$ ]]; then
	cat > ${PHP_INI_DIR}/conf.d/10-swoole.ini <<-EOF
		[swoole]
		extension=swoole.so
	EOF
fi
...
if [[ "${XDEBUG}" =~ [eE][nN][aA][bB][lL][eE] ]]; then
	if [[ "${XDEBUG_DEFAULT_CONF}" =~ [eE][nN][aA][bB][lL][eE] ]]; then
		cat >> ${PHP_INI_DIR}/conf.d/10-xdebug.ini <<-EOF
			[xdebug]
		...
		EOF
	elif [[ ! -f ${PHP_INI_DIR}/conf.d/10-xdebug.ini ]]; then
		echo >&2 "error:  missing Can't found ${PHP_INI_DIR}/conf.d/10-xdebug.ini"
		echo >&2 "Did you forget to add -v /xdebug_config_file:${PHP_INI_DIR}/conf.d/10-xdebug.ini"
		exit 1
	fi
fi
